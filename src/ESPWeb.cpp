...
background-color:rgb(240,240,240);\n\display:flex;\n\
  ...
  style += F("\n\#header{min-width: 600px; margin: 0 auto; padding:5px; border: 1px solid #ccc;}\n");
  style += F("input[type=submit], input[type=button] {display: inline-block; margin: 0 10px 0 0; padding: 5px 15px; min-width: 100px; font-size: 16px; font-family: serif; line-height: 1.8; appearance: none; box-shadow: none; border-radius: 0;");
  style += F("color: #fff; background-color: #6496c8; text-shadow: -1px 1px #417cb8; border: none;} input[type=submit]:focus, input[type=button]:focus {outline: none}\n input[type=submit]:hover, input[type=button]:hover {cursor: pointer; background-color: #346392; text-shadow: -1px 1px #27496d;}\n");
  style += F("input[type=text], input[type=password]{font-family: sans-serif; width: 20em; height: 2em; border: 0.1em solid #ccc; border-radius: 0.5em; background-color: #feffdc; padding: 0em 1em;} \n");
  style += F("select{font-family: sans-serif; width: 22em; border: 0.1em solid #ccc; border-radius: 0.5em; background-color: #feffdc; padding: 0em 1em;} \n");
  style += F("h3{text-align: center; color: #346392; font-size: 32px;}");
  style += F("label{font-weight:bold;}");
...
location.reload(true);\n\
  ...
  page += ESPWebBase::webPageScript(script);
  page += ESPWebBase::webPageBody();
  page += F("<div id='header'>\n\<form method=\"POST\" action=\"\" enctype=\"multipart/form-data\" onsubmit=\"if (document.getElementsByName('upload')[0].files.length == 0) { alert('No file to upload!'); return false; }\">\n\
<h3>SPIFFS</h3>\n\
<p>\n");
...
return true;\n\
  ...
  page += ESPWebBase::webPageScript(script);
  page += ESPWebBase::webPageBody();
  page += F("<div id='header'>\n\<form name=\"wifi\" method=\"GET\" action=\"");
...
setInterval(refreshData, 1000);\n");
  ...
  page += ESPWebBase::webPageScript(script);
  page += ESPWebBase::webPageBody();
  page += F("<div id='header'>\n\<form name=\"time\" method=\"GET\" action=\"");
  page += FPSTR(pathStore);
  page += F("\" onsubmit=\"return validateForm(this)\">\n\
<h3>Time Setup</h3>\n\
